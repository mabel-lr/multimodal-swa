...
from mymodel_speech import M3X1

import torch.nn.functional as F
import torch.nn as nn
import hparams_all_all as hp
...
import sys
import os
import paths # folder to torch tensors
...
label_count = t.zeros(4)
for op in trainset:
    op_label = t.load(labels_path + op + "/labels")
    phase3_idx = np.where(op_label == 3)[0][0]
    phase5_idx = np.where(op_label == 5)[0][-1]
    op_label = op_label[phase3_idx:phase5_idx]
    counts = t.bincount(op_label.int().squeeze())
    label_count += t.cat([counts[3:6], counts[8].view(1)])
...
class OPDataset(Dataset):
    ...
    def __getitem__(self,idx):
        c1 = self.data[0][idx].clone().detach().float().to(device)
        c2 = self.data[1][idx].clone().detach().float().to(device)
        c3 = self.data[2][idx].clone().detach().float().to(device)
        y = self.data[3][idx].clone().detach().long().to(device)
        return [c1, c2, c3, y]
    
    def __len__(self):
        return self.data[3].shape[0]

    def get_data(self, op):
        label = t.load(labels_path + op + "/labels")
        c1 = t.load(audio_path + op + "/physician_mic") 
        c2 = t.load(audio_path + op + "/assistant_mic")
        c3 = t.load(mfcc_path + op + "/mfcc")

        # get indexes to delete
        labels = label.cpu().detach().numpy()
        phase3_idx = np.where(labels == 3)[0][0]
        phase5_idx = np.where(labels == 5)[0][-1] 
        label_chop = label[phase3_idx:phase5_idx] - 3
        c1_chop = c1[phase3_idx:phase5_idx]
        c2_chop = c2[phase3_idx:phase5_idx]
        c3_chop = c3[phase3_idx:phase5_idx]

        mask2 = label_chop == 5
        label_chop[mask2] = 3
        return [c1_chop, c2_chop, c3_chop, label_chop]
...
for e in range(hp.epochs):
    if early_callback == False:
        ...
        for i_op, op in enumerate(trainset):
            ...
            y_prev = t.zeros((1, hp.compute_dim_speech), device = device) #(1, 128)
            i=0
            for c1, c2, c3, y in op_dataloader:
                optimizer.zero_grad()

                y_hat = model(c1, c2, c3, y_prev.clone().detach())
                ...
                y_prev[0, -min(hp.compute_dim_speech, c1.shape[0]):] = t.argmax(y_hat[:,:,-1], 
                                            dim = 1)[-min(hp.compute_dim_speech, c1.shape[0]):]
                i+=1
        ...
        for i_op, op in enumerate(validset):
            ...
            transition_idx = np.where(y_ground == 2)[0] # remove transition for evaluation
...
for i_op, op in enumerate(testset):
    ...
    for c1, c2, c3, y in op_dataloader:
        with t.no_grad():
            y_hat = best_model(c1, c2, c3, y_prev.clone().detach())
